...
- **Close + reopen** (trigger fires): `claimAllRewardsByPosition` → `removeLiquidity(shouldClaimAndClose:true)` → read post-close balances → **Jupiter Ultra swap to target X:Y ratio** → re-read balances → reopen at the LLM's price-bound-derived `{minBinId, maxBinId}`. LLM verb: `OPEN`. Reopen has two sub-paths gated on `window.width`:
  ...
  - `width > 70`: v1 `initializePosition` at width=70 (chunk-1 width only) in its own tx, then chunked add-liquidity txs (one per 70-bin chunk) built by calling the SDK helper `chunkDepositWithRebalanceEndpoint(..., isParallel=false)` directly — NOT `pool.addLiquidityByStrategyChunkable()`, which hardcodes `isParallel=true`. Each per-chunk `rebalanceLiquidity` ix auto-extends the position via inner-realloc as it deposits into bins beyond the position's current range (per-chunk realloc ≤ 70 × `POSITION_BIN_DATA_SIZE` = 7840 bytes, under Solana's 10240-byte inner-realloc cap). Chunked path necessary because the single-shot init ix's CPI realloc overruns the inner-realloc cap when init+add-liq run together at full width. **Do NOT pre-extend** the new position via `initializePosition2 + increasePositionLength2` — a pre-extended position forces the on-chain `ShrinkBoth` path to walk bins outside the current chunk during fee/reward processing and look up bin arrays that aren't in `remainingAccounts`, failing with `InvalidBinArray` (6027). Before the chunk loop, native deposit amounts (`totalXAmount` / `totalYAmount`) are re-clamped to `readDepositableBalance` so the post-init rent deduction does not cause the per-chunk wrap-SOL transfer to overdraw and fail simulation with `Transfer: insufficient lamports`.
