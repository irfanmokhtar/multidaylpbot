...
const Schema = z.object({
  ...
  // Bin-count tolerance for active-bin drift between simulation and on-chain execution
  // of the balanced rebalance path. SDK default is 3; raised to 10 to absorb drift
  // during the countdown window on volatile pools. Wider = fewer 6004 rejections.
  MAX_BIN_SLIPPAGE: z.coerce.number().int().positive().max(50).default(10),
  // Retry attempts on the balanced rebalance path when the on-chain ix rejects with
  // ExceededBinSlippageTolerance (6004). Each retry re-simulates against a fresh
  // active bin. 0 = no retry; default 2 = 1 retry after the initial attempt.
